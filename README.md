...
本次更新：重构邮件列表底部的历史同步操作，宽列表使用等宽完整按钮并显示已载入范围，窄列表自动切换为紧凑模式；明确区分“分批载入更早邮件”和“同步全部历史”，为全量同步增加耗时与可中断说明及二次确认；完善明暗主题、字号缩放和键盘操作，并修复 1024px 以下暗色界面顶栏被旧样式挤成两行、搜索框错位的问题。
