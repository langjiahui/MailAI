...
本次更新：修复邮件正文暂缺导致漏取的问题，保留失败邮件重试位置并继续接收其他邮件；历史同步期间穿插检查新邮件，失败或忙碌任务按约 30 秒调度周期重试，正常收信间隔仍为 5 分钟。新邮件逐封处理完成后即可通知，补齐窗口未隐藏时的系统通知，并统一通知偏好与去重。已确认处置、误报或删除的异常邮件退出小邮待处理提醒，相关提醒全部处理后折叠历史分析；修复列表刷新失败后未重试的问题。新增“分享 MailAI”，可复制最新版下载页或使用支持设备的系统分享。
