...
本次更新：优化 macOS 原生一体化窗口，保留系统红黄绿按钮、窗口拖动与缩放操作，统一深色、浅色和跟随系统主题。启动过渡与工作区背景、边界自然衔接；修复全屏时顶部操作被遮挡以及退出全屏后的工具栏恢复问题。窄窗口和大字号下自动使用紧凑操作入口，优化大窗口纯文本正文行宽；修复统一收件箱中不同账号同编号邮件同时高亮的问题。
