...
本次更新：帮助、关于与规则中心改用中性色背景，保留绿色强调开启状态和主要操作；规则按钮补齐悬停、按下与键盘聚焦反馈。小邮预览改为连续文稿，主题单独填写，正文操作集中在底部，并按现有内容显示填入、替换或追加；填写时增加短暂逐字效果，兼容减少动态效果设置。删除同步失败后最多再重试两次，随后保留本地删除状态并结束待处理任务，支持恢复本地邮件。
