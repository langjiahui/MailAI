...
本次更新：交互反馈与英文界面第二轮清扫。所有"处理中"按钮统一带旋转指示；语义索引重建新增实时进度条（下载模型阶段滚动提示、建索引阶段显示具体进度，切换页签后自动恢复）；诊断结果逐条错峰浮现。英文界面补齐约 380 处遗漏文案，覆盖多账号侧栏文件夹、阅读窗格元信息与研判浮层、通讯录、附件中心、待办中心、安全看板图表与关联研判、规则中心全部场景卡与 46 条检测规则的名称/说明/命中条件、每日日报、账号页徽标、首次引导等动态渲染区域；运行诊断在英文界面下直接返回英文检查项与修复指引。修复了顶部"安全看板/规则中心"按钮在切换视图后回落为中文的问题。
