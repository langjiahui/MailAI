...
本次更新：点击侧边栏各邮箱的收件箱/文件夹时立即高亮选中态，不再等待网络往返，迟滞感消除（失败自动回滚）；同步按钮加载时不再出现双重转圈图标；设置中备份记录列表完成国际化，英文界面显示 Migration / Save as / Delete / Restore 等，时间格式随语言切换；待办中心修复英文长文案按钮溢出卡片右缘的问题，操作按钮等宽收进卡片内，并补齐暗色主题配色。
