...
本次更新：全局动效层上线，交互质感统一升级。所有按钮、导航、卡片统一回弹按压手感；邮件列表悬停右移、风险条加粗；打开邮件时阅读区错峰浮现、安全结论徽章弹性弹入；写邮件、通讯录、附件中心等弹窗统一背景渐入 + 弹性落位；列表与阅读区加载改为扫光骨架屏；待办与附件计数数字滚动过渡；高风险警示条增加克制的呼吸微光；安全看板卡片悬浮微浮、指标错峰入场；输入框聚焦光环平滑浮现；滚动条改用品牌绿调。全程遵循 prefers-reduced-motion，系统开启减弱动画时自动全部关闭；写邮件签名选择器对齐工具条视觉并补齐暗色主题。
