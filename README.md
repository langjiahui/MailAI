...
本次更新：优化邮件阅读区的多选预览与操作栏：多封邮件以自适应、错落层叠的方式展示最后选中的邮件预览；单封邮件保持专注阅读。邮件、AI 与风险处置功能改为统一的平铺工具栏，仅用细分隔线区分，空间不足时自动收起文字仅保留图标。更新双向“往来邮件”图标，移除无效的“会话通知”操作，并完善邮箱连接诊断的引导反馈。
