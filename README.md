...
本次更新：完善 macOS 窗口交互，支持双击顶部空白区域或 MailAI 标识将窗口填满当前屏幕可用区域，再次双击恢复原尺寸，搜索框与操作按钮保持独立响应。优化全屏左上角布局，收起窗口控制按钮后使用紧凑图标与名称，消除多余留白；进入和退出全屏时保持搜索框位置稳定，自动恢复普通窗口的按钮预留空间。深色、浅色主题均已适配，并补充窗口切换与布局回归检查。
