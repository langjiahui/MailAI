...
本次更新：语义检索开启后会在后台建立并随收信更新索引；具体邮箱的顶部搜索可在关键词结果不足时补充相关邮件，拼音搜索也能命中正文中的中文姓名。优化邮件列表、搜索框、任务与发件箱、阅读区和界面过渡；完善已发送邮件删除与再次编辑后舍弃草稿的处理。语义模型仅在本机处理索引，搜索或索引失败时不影响普通收信与关键词检索。
