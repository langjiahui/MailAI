...
本次更新：修复快速切换不同账号收件箱、已发送和草稿箱时旧请求覆盖最后一次点击的问题，侧栏会立即显示选中状态且始终以最后一次操作为准；统一收件箱会正确清理旧搜索结果，服务端文件夹缓存按账号隔离；小邮现在会明确区分读取邮件、读取图片或附件、检索、分析、重试和本地降级等阶段，停止后重试不再重复问题；优化写信 AI 在宽屏、中等窗口和移动端的布局，并修复新手引导遮挡写信弹窗操作的问题。
