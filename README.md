...
本次更新：重设计待办中心，串联任务分类、安排与提醒记录，支持按当前视图批量完成。删除同步失败增加暂停与恢复入口，保留服务器处理阶段。附件中心新增可记忆的紧凑列表与卡片切换；阅读工具栏缩小按钮、弱化边框并按宽度收纳次要操作。小邮生成结果分开展示主题、正文与落款，填写位置更直观。统一按钮、表单与反馈状态，优化设置、通讯录、规则、签名及多个弹窗；修复联系人编辑被裁切和大字号下顶栏重叠，适配浅色、深色和窄窗口。
