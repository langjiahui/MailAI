...
本次更新：版本发布说明会从 README 自动同步到更新弹窗、设置中的“版本与更新”和 GitHub 发布页；下载安装时新增实时百分比、下载量、速度及动态校验进度；Windows 和 macOS 完成安装后会尽力自动重新打开 MailAI，启动失败不会影响安装结果。同时将残留的旧产品定位措辞替换为通用产品说明。
