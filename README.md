...
本次更新：新增 Windows 与 macOS 待办系统提醒和提醒记录，支持快捷延后、完成及失败重试；统一确认弹窗、保存反馈与连接异常提示，待办刷新保留编辑内容和焦点。优化附件中心的文件图标、预览与下载入口，修复大量附件下卡片挤压重叠，并适配窄窗口、暗色和放大字号。优化安全看板及工作简报样式，去除 AI 写信“暂无”参考项的勾选残影。源码本地启动自动检测端口占用并切换可用端口，同时保留同一数据目录的重复启动保护。
