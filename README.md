...
本次更新：完善大附件本地任务恢复，上传失败后可保存关闭草稿，重新打开继续处理；任务与发件箱展示删除同步进度并支持失败重试。搜索增加范围、进行中和失败反馈，优化本地语义检索的性能与稳定性。AI 写信支持撤销最近一次填入，重新生成失败保留上一份草稿；完善主题、正文及签名处理、键盘焦点和逐层关闭体验。修复上传切换草稿时的竞争问题，修复 Windows 重复启动时的文件锁冲突，并加强跨平台安装包校验。
