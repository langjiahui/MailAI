...
def main():
    ...
    with patch.object(outgoing_guard.db, "contact_history", return_value={"user@example.com": 3}), \
    ...
    with patch.object(outgoing_guard.db, "get_email", return_value=risky_source), \
         patch.object(outgoing_guard.db, "contact_history", return_value={"user@example.com": 2}):
